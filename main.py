...
linksInThread = [] #target amount of links by every thread
currentLinksInThread = [] #current amount of links by every thread
...
def scrape_links(thread):
    ...
    while True:
        ...
        try:
            url = 'http://goo.gl/'
            adress = ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(6))
            print(adress)
            url += adress
